...
### 8단계
- 프로젝트를 스프링 부트로 설정한다.
- 게임을 시작할 시 보드를 초기화하는 api를 만든다.
- 기물 이동 api를 만든다.
- 게임 종료시 결과를 리턴하는 api를 만든다.
- 킹이 잡히는 경우 게임을 종료하는 기능 구현
- 킹이 잡힐 경우 기존과 다른 메세지를 전송
- 해당 메세지를 받은 프론트에서 결과 api를 요청
...
- Api를 구현할때 서버에서는 Dto가 제대로 만들어졌지만 클라이언트에서 데이터를 확인 하지 못하는 문제가 발생했습니다.
 원인은 Api응답을 담는 Result에서 getter를 만들어주지 않아 생긴 문제였습니다.Jackson이 직렬화 할때 Getter가 존재하는지 확인해서 직렬화를 수행하기 때문 발생한 문제였습니다.
- npm 오류로 인해 리액트를 사용하지 못하는 문제가 발생했었다. 이를 해결하기 위해 vue.js를 cdn방식으로 사용하여 npm없이도 프론트를 구축하도록 했습니다.
- 프론트에서 시작 버튼을 누른 후 이전 판의 정보가 나오는 문제가 있었습니다. 처음에는 캐시나 쿠키의 문제인 줄 알았지만 서버에서 보드나 게임 객체가 싱글톤 객체인 것이 문제였습니다.
 서버는 계속 띄워져 있으니 이미 존재하는 보드에 계속 피스들이 초기화되어 추가 되었고 이로 인해 이전 판의 정보가 나오게 되었습니다.
 이를 해결하기 위해 start api를 호출하면 모든 보드의 내용을 삭제한 후 초기화를 하도록 수정하였습니다.